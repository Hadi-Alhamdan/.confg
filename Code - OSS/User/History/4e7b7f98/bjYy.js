...
// --- API Function for Daily Score & Status ---

// ... (fetchDailyScore function should already be here) ...

/**
 * Marks or unmarks a specific date as a rest day.
 * @param {string} date - The date in 'YYYY-MM-DD' format.
 * @param {boolean} isRestDay - True to mark as rest day, false to unmark.
 * @returns {Promise<Object>} A promise that resolves to the API response,
 *                            which includes the updated score data for that day.
 */
async function updateDailyStatus(date, isRestDay) {
    if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
        throw new Error("Invalid or missing date. Use YYYY-MM-DD format.");
    }
    if (typeof isRestDay !== 'boolean') {
        throw new Error("'isRestDay' must be a boolean value.");
    }

    const response = await fetch(`${API_BASE_URL}/daily-status/${date}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ is_rest_day: isRestDay }),
    });
    // The backend /api/daily-status/:date endpoint returns the updated score data
    // which is useful for immediately refreshing the UI if needed.
    return handleResponse(response);
}
...
// --- API Functions for History & Charts ---

/**
 * Fetches daily total scores within a given date range for charting.
 * @param {string} startDate - The start date in 'YYYY-MM-DD' format.
 * @param {string} endDate - The end date in 'YYYY-MM-DD' format.
 * @returns {Promise<Array<Object>>} A promise that resolves to an array of score objects 
 *                                   (e.g., [{ date: 'YYYY-MM-DD', total_daily_score: 75.5 }, ...]).
 */
async function fetchScoresHistory(startDate, endDate) {
    ...
    const response = await fetch(`${API_BASE_URL}/scores/history?startDate=${startDate}&endDate=${endDate}`);
    const data = await handleResponse(response);
    return data.data; // The array of score objects is in the 'data' property
}

/**
 * Fetches completion history for a specific habit within a given date range.
 * @param {number} habitId - The ID of the habit.
 * @param {string} startDate - The start date in 'YYYY-MM-DD' format.
 * @param {string} endDate - The end date in 'YYYY-MM-DD' format.
 * @returns {Promise<Array<Object>>} A promise that resolves to an array of habit completion objects
 *                                   (e.g., [{ date: 'YYYY-MM-DD', completed: true, weight_at_completion: 0.5 }, ...]).
 */
async function fetchHabitHistory(habitId, startDate, endDate) {
    if (!habitId || !startDate || !endDate) {
        throw new Error("habitId, startDate, and endDate are required.");
    }
    const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    if (!dateRegex.test(startDate) || !dateRegex.test(endDate)) {
        throw new Error("Invalid date format. Please use YYYY-MM-DD.");
    ...
    const response = await fetch(`${API_BASE_URL}/habits/${habitId}/history?startDate=${startDate}&endDate=${endDate}`);
    const data = await handleResponse(response);
    return data.data;
}
...
async function exportData() {
    ...
    if (!response.ok) {
        // We can't use handleResponse directly because it expects JSON for errors
        // and this might be a network error before JSON is formed.
        const errorText = await response.text();
        console.error('API Error exporting data:', response.status, errorText);
        alert(`Export Error: Request failed with status ${response.status}. ${errorText}`);
        throw new Error(`Export failed: ${response.status}`);
    }
    return response.blob(); // Get data as a Blob to save as a file
}
...
async function importData(jsonData) {
    ...
    return handleResponse(response); // Use existing helper for JSON response
}



/**
 * Un-marks a habit as complete for a specific date (deletes the completion record).
 * @param {number} habitId - The ID of the habit.
 * @param {string} date - The date in 'YYYY-MM-DD' format for which to undo completion.
 * @returns {Promise<Object>} A promise that resolves to the success message from the API.
 */
async function unmarkHabitComplete(habitId, date) {
    if (!habitId || !date) {
        throw new Error("habitId and date are required to unmark habit completion.");
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
        throw new Error("Invalid date format. Please use YYYY-MM-DD.");
    }

    const response = await fetch(`${API_BASE_URL}/habits/${habitId}/complete?date=${date}`, {
        method: 'DELETE',
...
/**
 * Saves the daily notes for a specific date.
 * @param {string} date - The date in 'YYYY-MM-DD' format.
 * @param {string} notes - The notes content.
 * @returns {Promise<Object>} A promise that resolves to the success message from the API.
 */
async function saveDailyNotes(date, notes) {
    if (!date) throw new Error("Date is required to save notes.");

    const response = await fetch(`${API_BASE_URL}/daily-notes/${date}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
