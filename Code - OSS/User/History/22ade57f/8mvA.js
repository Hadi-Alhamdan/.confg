...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (applyDateRangeBtn) {
        applyDateRangeBtn.addEventListener('click', updateAllCharts);
    }

    if (predefinedRangeButtons) {
        predefinedRangeButtons.forEach(button => {
            button.addEventListener('click', () => {
                console.log(`Predefined range button clicked: ${button.dataset.range}`);
                const range = button.dataset.range;
                const endDate = new Date(); // Use local 'today' as end for most ranges
                ...
                updateAllCharts(); // Call the central update function
            });
        });
    }

    if (habitSelectForChart) {
        habitSelectForChart.addEventListener('change', loadHabitChartData); // Only needs to reload habit chart
    }
    ...
    function initializeHistoryPage() {
        console.log("History page being initialized/refreshed.");
        if (!historyPagePopulated) { // Run one-time population tasks only once
            setDefaultDateRange();
            populateHabitSelect();
            historyPagePopulated = true;
        }
        updateAllCharts(); // Load charts with current/default date range
        loadDailyScoreDetails(); // Load initial score details
    }

    // Expose the initialization function to be called by app.js
    // This is more reliable than MutationObserver for view changes managed by app.js
    window.chartJsModule = {
        initializeOrRefreshCharts: initializeHistoryPage
    };

    // Initial check if history section is ALREADY visible when this script runs
    // (e.g. if user refreshes on history page, or if it's the default view for some reason)
    const historySection = document.getElementById('history-section');
    if (historySection && historySection.style.display !== 'none' && historySection.offsetParent !== null) {
        console.log("History section initially visible, initializing charts.");
