...
async function recalculateDailyScore(date) {
    try {
        // ... (fetch settings, prevDayStreak - keep as is) ...
        const settings = await getDb('SELECT * FROM Settings WHERE id = 1');
        const dayOfWeek = new Date(date + 'T00:00:00').getDay();
        let isRestDay = false;
        if (settings) {
            const restDayKeys = [
                'rest_day_sunday',    // Index 0
                'rest_day_monday',    // Index 1
                'rest_day_tuesday',   // Index 2
                'rest_day_wednesday', // Index 3
                'rest_day_thursday',  // Index 4
                'rest_day_friday',    // Index 5
                'rest_day_saturday'   // Index 6
            ];
            isRestDay = settings[restDayKeys[dayOfWeek]] === 1;
        }
