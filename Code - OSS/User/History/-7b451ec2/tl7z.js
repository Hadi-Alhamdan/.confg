...
async function recalculateStreaksForward(startDateToRecalculateFrom) {
    ...
    while (currentDate <= stopDate) {
        ...
        const existingScoreData = await getDb("SELECT total_daily_score, streak_bonus_component, is_manually_marked_rest_day FROM DailyScores WHERE date = ?", [currentDateStr]);

        if (existingScoreData) {
            scoreToUse = existingScoreData.total_daily_score - existingScoreData.streak_bonus_component; // Base score
            isRestDay = existingScoreData.is_manually_marked_rest_day === 1;
            oldBonus = existingScoreData.streak_bonus_component;
        } else {
            // If no score entry exists, it means this day had no activity that would have
            // triggered a score calculation yet. We need to calculate its score from scratch.
            // recalculateDailyScore will create the DailyScores entry and also calculate
            // its initial streak based on ITS previous day.
            console.log(`No DailyScore for ${currentDateStr}. Triggering full recalculateDailyScore to establish baseline.`);
            const initialScoreCalculation = await recalculateDailyScore(currentDateStr);
            // Now use the results from this full calculation for the current loop iteration
            scoreToUse = initialScoreCalculation.total_daily_score - initialScoreCalculation.streak_bonus_component;
            isRestDay = initialScoreCalculation.is_manually_marked_rest_day === 1; // or initialScoreCalculation.is_manually_marked_rest_day (if boolean)
            oldBonus = initialScoreCalculation.streak_bonus_component;
        }
