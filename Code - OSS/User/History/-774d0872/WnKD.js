...
const db = require('../database.js'); // Adjust path if your db file is named differently or located elsewhere
...
router.post('/:id/complete', (req, res) => {
    const habitId = parseInt(req.params.id);
    const completionDate = req.body.date || getCurrentDateYYYYMMDD(); // Default to today if not provided
    ...
    // 1. Fetch the current weight of the habit
    db.get("SELECT current_weight FROM Habits WHERE id = ?", [habitId], (err, habit) => {
        if (err) {
            return res.status(500).json({ error: `Error fetching habit: ${err.message}` });
        }
        if (!habit) {
            return res.status(404).json({ message: "Habit not found." });
        }
        ...
        // 2. Insert into HabitCompletions
        // The UNIQUE constraint on (habit_id, completion_date) will prevent duplicates for the same day.
        const insertSql = `INSERT INTO HabitCompletions (habit_id, completion_date, weight_at_completion)
                           VALUES (?, ?, ?)
                           ON CONFLICT(habit_id, completion_date) DO UPDATE SET
                           weight_at_completion = excluded.weight_at_completion,
                           created_at = CURRENT_TIMESTAMP`; // Update if it already exists (e.g. if weight changed and re-marked)

        db.run(insertSql, [habitId, completionDate, weightAtCompletion], function(err) {
            if (err) {
                return res.status(500).json({ error: `Error marking habit complete: ${err.message}` });
            }
            // TODO LATER: After this, trigger recalculateDailyScore(completionDate)
            console.log(`Habit ${habitId} marked complete for ${completionDate} with weight ${weightAtCompletion}. Daily score recalculation pending.`);
