...
router.delete('/:id/complete', async (req, res) => {
    ...
    try {
        ...
        console.log(`[DEBUG] Date string for UTC parsing in HABIT UNCOMPLETE: '${date}'`);
        const year = parseInt(date.substring(0, 4));
        const month = parseInt(date.substring(5, 7)); // Month from string is 1-12
        const day = parseInt(date.substring(8, 10));
        console.log(`[DEBUG] Parsed parts: Year=${year}, Month=${month}, Day=${day}`);

        if (isNaN(year) || isNaN(month) || isNaN(day)) {
            console.error("[DEBUG] Date parsing resulted in NaN for year, month, or day.");
            throw new Error("Internal error: Date component parsing failed.");
        }

        // Create Date object for UTC
        // Date.UTC expects month to be 0-11
        const dayAfterCompletionObj = new Date(Date.UTC(year, month - 1, day));
        console.log(`[DEBUG] Initial dayAfterCompletionObj (UTC midnight of input date): ${dayAfterCompletionObj.toISOString()}`);

        if (isNaN(dayAfterCompletionObj.getTime())) {
            console.error(`[DEBUG] dayAfterCompletionObj is an invalid date for input: ${date}. UTC parts: Y=${year}, M=${month - 1}, D=${day}`);
            throw new Error("Internal error: Could not parse date for streak forward calculation.");
        }

        dayAfterCompletionObj.setUTCDate(dayAfterCompletionObj.getUTCDate() + 1);
        console.log(`[DEBUG] dayAfterCompletionObj after adding 1 day (UTC): ${dayAfterCompletionObj.toISOString()}`);

        if (isNaN(dayAfterCompletionObj.getTime())) {
            console.error(`[DEBUG] dayAfterCompletionObj became invalid after adding 1 day.`);
            throw new Error("Internal error: Date became invalid after incrementing.");
        }

        const nextDayStr = dayAfterCompletionObj.toISOString().slice(0, 10);
        console.log(`[DEBUG] nextDayStr for recalculateStreaksForward: ${nextDayStr}`);
...
router.get('/:id/history', async (req, res) => { // Add async
    ...
    function getDatesInRange(start, end) {
        const dates = []; // Initialize as an array
        ...
        if (isNaN(currentDate.getTime()) || isNaN(stopDate.getTime())) {
            console.error("Invalid date string provided to getDatesInRange after parsing:", start, end);
            return []; // Return an empty array on error to prevent .map error
        }
        ...
        return dates; // Crucially, always return the array
    }
    ...
    try {
        ...
        const chartData = allDatesInRange.map(dateStr => { // Ensure this is the detailed version
            const isCompleted = completionsMap.has(dateStr);
            const weight = completionsMap.get(dateStr); // Will be undefined if not in map
            return {
                date: dateStr,
                completed: isCompleted,
                weight_at_completion: isCompleted ? weight : null // Only set weight if completed
            };
        });
