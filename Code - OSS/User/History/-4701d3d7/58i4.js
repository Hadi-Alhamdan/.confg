...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleHabitListClick(event) {
        ...
        const habitId = parseInt(habitItem.dataset.habitId);
        const today = new Date().toISOString().slice(0, 10); // Assuming completions are for 'today' in this UI
        ...
        if (target.classList.contains('complete-btn')) {
            ...
            if (isCurrentlyCompleted) {
                // --- UNDO LOGIC ---
                try {
                    await unmarkHabitComplete(habitId, today); // Use today's date
                    // No need to manually update todaysCompletions here if loadAllHabits re-fetches them
                    await loadAllHabits(); // Reload to reflect the change and get updated score
                    document.body.dispatchEvent(new CustomEvent('dataChanged', { detail: { date: today } }));
                } catch (error) {
                    console.error(`Failed to un-mark habit ${habitId} as complete:`, error);
                    alert("Failed to undo habit completion."); // Simple feedback
                }
            } else {
                // --- COMPLETE LOGIC (as before) ---
                try {
                    await markHabitComplete(habitId, today); // Use today's date
                    // No need to manually update todaysCompletions here if loadAllHabits re-fetches them
                    await loadAllHabits();
                    document.body.dispatchEvent(new CustomEvent('dataChanged', { detail: { date: today } }));
                } catch (error) {
            ...
            // ... (existing archive logic - ensure it calls loadAllHabits and dispatches dataChanged)
            const isArchiving = !habitItem.parentElement.id.includes('archived');
            try {
                await updateHabit(habitId, { is_archived: isArchiving });
                await loadAllHabits();
            ...
            // ... (existing delete logic - ensure it calls loadAllHabits and dispatches dataChanged)
            if (confirm('Are you sure you want to permanently delete this habit and all its history?')) {
                try {
                    await deleteHabit(habitId);
                    await loadAllHabits();
