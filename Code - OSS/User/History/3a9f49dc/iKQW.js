...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const markTodayRestCheckbox = document.getElementById('mark-today-rest-checkbox');
        ...
        try {
            ...
            if (scoreData && typeof scoreData.total_daily_score === 'number') {
                ...
                if (date === getTodayString() && markTodayRestCheckbox) {
                    // scoreData.is_manually_marked_rest_day will be 1 for true, 0 for false from SQLite
                    markTodayRestCheckbox.checked = scoreData.is_manually_marked_rest_day === 1;
                }
    ...
    function showView(viewIdToShow) {
        ...
        navButtons.forEach(button => {
            if (button.dataset.view === viewIdToShow) {
                button.classList.add('active'); // Your JS adds this
            } else {
    ...
    if (markTodayRestCheckbox) {
        markTodayRestCheckbox.addEventListener('change', async () => {
            const today = getTodayString();
            const isRest = markTodayRestCheckbox.checked;
            try {
                // Call API to update the rest day status for today
                const response = await updateDailyStatus(today, isRest); // from api.js
                console.log("Daily status update response:", response);

                // The backend already recalculates. The response.data contains the updated score.
                // We dispatch 'dataChanged' so displayDashboardScore re-fetches and updates everything consistently.
                document.body.dispatchEvent(new CustomEvent('dataChanged', { detail: { date: today } }));

            } catch (error) {
                console.error(`Failed to mark today (${today}) as rest=${isRest}:`, error);
                // Revert checkbox on failure
                markTodayRestCheckbox.checked = !isRest;
                alert("Failed to update rest day status. Please try again.");
            }
        });
    }
